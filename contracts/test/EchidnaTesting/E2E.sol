...
contract E2E is PropertiesAsserts {
    ...
    uint256 public constant START_TIMESTAMP = 1706745600;
    uint256 public constant START_BLOCK = 17336000;
    ...
    function stake(uint8 actorIndex, uint256 amount) public {
        ...
        emit LogString(string.concat(
            "Staked ", amount.toString(), " tokens by actor ", uint256(actorIndex).toString()
        ));
        
        assertEq(
            staker.stakedBalanceOf(address(actors[actorIndex])),
            preStaked + amount,
            "Staked balance mismatch"
        );
        assertEq(
            staker.totalStaked(),
            preTotal + amount,
    ...
    function withdraw(uint8 actorIndex, uint256 amount) public {
        ...
        emit LogString(string.concat(
            "Withdrew ", amount.toString(), " tokens by actor ", uint256(actorIndex).toString()
        ));
        ...
        assertEq(
            stakingToken.balanceOf(address(actors[actorIndex])),
            preBalance + amount,
    ...
    function claimRewards(uint8 actorIndex, uint8 tokenIndex) public {
        ...
        emit LogString(string.concat(
            "Claimed ", earned.toString(), " rewards by actor ", uint256(actorIndex).toString()
        ));
        
        assertEq(
            IERC20(rewardToken).balanceOf(address(actors[actorIndex])),
            preBalance + earned,
            "Reward claim mismatch"
        );
        assertEq(
            staker.earned(address(actors[actorIndex]), rewardToken),
            0,
            "Rewards not reset"
        );
    }

    function claimAllRewards(uint8 actorIndex) public {
        ...
        emit LogString(string.concat(
            "Claimed all rewards by actor ", uint256(actorIndex).toString()
        ));
        
        if (earned1 > 0) {
            assertEq(
                rewardToken1.balanceOf(address(actors[actorIndex])),
                preBalance1 + earned1,
                "Token1 claim mismatch"
            );
        }
        if (earned2 > 0) {
            assertEq(
                rewardToken2.balanceOf(address(actors[actorIndex])),
                preBalance2 + earned2,
                "Token2 claim mismatch"
            );
        }
    }
    ...
    function addReward(uint8 tokenIndex, uint256 totalRewards, uint256 duration) public {
        ...
        emit LogString(string.concat(
            "Added reward: token=", Strings.toHexString(rewardToken),
    ...
    /* ================================================================
       ...
       ================================================================ */
    function maxStake_correctMax(uint8 actorIndex) public {
    ...
    function maxWithdraw_correctMax(uint8 actorIndex) public {
        ...
        assertEq(
            stakingToken.balanceOf(address(actors[actorIndex])),
            preBalance + preStaked,
            "Token balance mismatch"
        );
    }

    function maxStake_correctReturnValue(uint8 actorIndex) public {
    ...
    function reward_claim_consistency(uint8 actorIndex) public {
        ...
        if (earned1Before > 0 || earned2Before > 0) {
            emit LogString(string.concat(
                "Claimed rewards: ", 
                earned1Before.toString(), " + ", 
                earned2Before.toString()
            ));
        }
    }
