...
abstract contract Properties is BeforeAfter, Asserts {
    ...
    function invariant_CORE_INV_B() internal {
    ...
    function invariant_CORE_INV_C() internal {
        lte(staking.protocolFee(), staking.MAX_FEE(), CORE_GPOST_C);
    }

    function invariant_CORE_INV_D() internal {
    ...
    function invariant_REWARD_INV_A() internal {
    ...
    function invariant_LOCK_INV_A() internal {
        address[] memory actors = _getActors();
        uint256 totalLocked;
        for (uint256 i; i < actors.length; ++i) {
